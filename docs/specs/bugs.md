...
## BUG-022: 土日は株価が「本日」と表示されるが実際は前営業日
...
| Severity | medium |
| Status | resolved |
| 対象ファイル | `src/freshness.ts`, `src/FreshnessBanner.tsx`, `docs/specs/data_sources.md` |
| 発見経路 | データ更新後の表示確認（土曜日に実行したため顕在化） |

### 再現手順
1. 金曜の終値を取得した状態にする（`pipelineMeta.priceDate = '2026-09-25'`）
2. **土曜または日曜**にダッシュボードを開く

### 期待する動作
「前営業日」など、前日の終値であることが分かる表示。

### 実際の動作
鮮度バナーの株価が **「本日」** と表示され、すぐ下に **`2026-09-25`** と出る。
「本日」と言いながら日付は前日という、**表示そのものが矛盾した状態**。

### 原因
```ts
// 修正前 FreshnessBanner.tsx
const priceHeadline = priceBizDays <= 0 ? '本日' : `${priceBizDays}営業日前`;
```

`businessDaysSince()` は土日を数えないため、金曜から土曜への経過営業日数は **0** になる。
これを「今日」と読み替えていた。**営業日数 0 は「今日」ではなく「まだ次の営業日が来ていない」**
という意味でしかない。

**鮮度の判定（緑）自体は正しい。** 土曜時点で金曜終値は最新であり、
これ以上新しい株価は存在しない。誤っていたのは文言だけ。
ただし **土日は毎週必ず発生する**ため、目にする頻度は高い。

BUG-005 / BUG-011 / BUG-013 と同じ「画面が事実と違うことを言う」系統で、
[[feedback-verify-display-vs-data]] にあたる。

### 対応（2026-09-26）
- `freshness.ts` に `priceFreshnessLabel()` を追加し、文言の組み立てを集約した。
  色（しきい値判定）と文言を別の関数に分け、**営業日数とカレンダー日数の両方**を見る。

| 状況 | 表示 |
|---|---|
| カレンダー日数 0 | 本日 |
| 営業日数 0 かつカレンダー日数 1 以上（土日） | **前営業日** |
| それ以外 | N営業日前 |
| 基準日が未来 | 基準日が未来（要確認） |
| 基準日が無い | 取得日不明 |

- 未来日付を「本日」と言わない分岐も入れた（`data.ts` の生成ミスを隠さないため）。
- ツールチップに経過営業日数と「土日は前営業日の終値のままになる」旨を追記した。

### 検証
ブラウザの時計を6つの日付に進めて表示を確認した（Playwright の `addInitScript` で `Date` を差し替え）。
`priceDate = 2026-09-25`（金）固定。

| 時計 | 曜日 | 表示 | 判定 |
|---|---|---|---|
| 2026-09-25 16:00 | 金・引け後 | 本日 | ✓ |
| 2026-09-26 | 土 | **前営業日** | ✓ |
| 2026-09-27 | 日 | **前営業日** | ✓ |
| 2026-09-28 | 月 | 1営業日前 | ✓ |
| 2026-10-01 | 木 | 4営業日前 | ✓ |
...
`tsc --noEmit` / `vite build` 通過。コンソールエラーなし。

### 残る既知の制限
祝日は営業日として数えるため、祝日を挟むと実際より1日古く表示される
（「実際より古く見える」方向なので安全側。`dashboard.md` の既知課題に記載済み）。
